...
class Arguments:
    def __init__(
        ...
        self.img_name_unique = img_name_unique
        ...
        self.u_classes = u_classes
        self.num_model_classes = len(self.u_classes)     
...
class InferenceTask:
    ...
    def execute(self, received_image) -> np.ndarray:
        """
        ...
        return self.segment_img(received_image)
    ...
    def segment_img(self, received_image) -> np.ndarray:
        
        self.model.eval()

        rgb_img = cv2.cvtColor(received_image, cv2.COLOR_BGR2RGB)
        rgb_img = np.float32(rgb_img)
        ...
        # overlaid_img = np.array([1, 2, 3, 4])
        # overlaid_img = None
        
        return pred_label_img, overlaid_img
